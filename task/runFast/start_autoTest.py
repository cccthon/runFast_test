...
#读入配置文件中数据
commonData = QSCommon.commonData()
#####################################
#定义需要参与连跑的测试用例
#将account目录下的所有FM开头的python脚本加入测试套
testSuite = QSCommon.testSuite('../../test', 'QS*.py')
# tester = QSCommon.get_computer_info()
for i in testSuite:
...
reportDir = '../../report' + '/' + currYear + currMonth + '/' + currDay + '/' + 'new'
#创建报告目录
QSCommon.mkdir(reportDir)
...
if __name__ == '__main__':
    ...
    testRunner = HTMLTestRunnerEN.HTMLTestRunner(
        ...
        description=commonData['reportFitTitle'] + commonData['hostName'],
        tester = 'tester')
...
shutil.copyfile(reportDir + '/index.html', '../../report' + '/' + currYear + currMonth + '/' + currDay + '/' + currTime + '.html')

#获取测试结果
status = QSCommon.getCharInFile(reportDir + '/index.html','</strong>')
...
if localtime.tm_hour < 7 or localtime.tm_hour > 22:
    print(reportToAll)
    QSCommon.sendMail(mmeText,fromAddr,reportToAll,subject,smtpServer,passwd)
else:
    print(reportToTest)
    QSCommon.sendMail(mmeText,fromAddr,reportToTest,subject,smtpServer,passwd)
